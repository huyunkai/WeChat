...
#import "YCYContactsViewController.h"
#import "YCYChatViewController.h"

@interface YCYContactsViewController()<NSFetchedResultsControllerDelegate>{
...
@implementation YCYContactsViewController
...
-(void)loadFriends2{
    ...
    NSManagedObjectContext *context = [YCYXMPPTool sharedYCYXMPPTool].rosterStorage.mainThreadManagedObjectContext;
    ...
    // 3.设置过滤和排序
    // 过滤当前登录用户的好友
    NSString *jid = [YCYUserInfo sharedYCYUserInfo].jid;
...
-(void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath{
    if (editingStyle == UITableViewCellEditingStyleDelete) {
        ...
        [[YCYXMPPTool sharedYCYXMPPTool].roster removeUser:freindJid];
    }
}
...
-(void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender{
    ...
    if ([destVc isKindOfClass:[YCYChatViewController class]]) {
        YCYChatViewController *chatVc = destVc;
