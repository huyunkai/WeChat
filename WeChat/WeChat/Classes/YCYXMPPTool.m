...
#import "YCYXMPPTool.h"
NSString *const YCYLoginStatusChangeNotification = @"YCYLoginStatusNotification";
/*
 * 在AppDelegate实现登录
 ...
 4. 授权成功后，发送"在线" 消息
 */
@interface YCYXMPPTool ()<XMPPStreamDelegate>{
...
@implementation YCYXMPPTool


singleton_implementation(YCYXMPPTool)
...
-(void)connectToHost{
    ...
    if (self.isRegisterOperation) {
        user = [YCYUserInfo sharedYCYUserInfo].registerUser;
    }else{
        user = [YCYUserInfo sharedYCYUserInfo].user;
    }
...
-(void)sendPwdToHost{
    ...
    // 从单例里获取密码
    NSString *pwd = [YCYUserInfo sharedYCYUserInfo].pwd;
...
-(void)postNotification:(XMPPResultType)resultType{
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:YCYLoginStatusChangeNotification object:nil userInfo:userInfo];
}
...
-(void)xmppStreamDidConnect:(XMPPStream *)sender{
    ...
    if (self.isRegisterOperation) {//注册操作，发送注册的密码
        NSString *pwd = [YCYUserInfo sharedYCYUserInfo].registerPwd;
...
//    self.window.rootViewController = storyboard.instantiateInitialViewController;
    ...
    //4.更新用户的登录状态
    [YCYUserInfo sharedYCYUserInfo].loginStatus = NO;
    [[YCYUserInfo sharedYCYUserInfo] saveUserInfoToSanbox];
